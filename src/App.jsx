...
const SITE_URL = 'https://vetorglobal.com.br'
...
const removeSeoJson = () => document.getElementById('vetor-article-jsonld')?.remove()
const setArticleSeo = article => {
  const title = limparTitulo(article?.title || '', fonteEditorial(article)) || 'Notícia | Vetor Global'
  const description = resumoLimpo(article?.summary || '', article?.title || '') || title
  const url = SITE_URL + '/noticia/' + encodeURIComponent(article.id)
  document.title = title + ' | Vetor Global'
  setCanonical(url)
  setMeta('description', description.slice(0, 155))
  setMeta('og:url', url, true); setMeta('og:title', title, true); setMeta('og:description', description.slice(0, 200), true); setMeta('og:type', 'article', true)
  if (article?.image_url) setMeta('og:image', article.image_url, true)
  removeSeoJson()
  const script=document.createElement('script'); script.id='vetor-article-jsonld'; script.type='application/ld+json'; script.textContent=JSON.stringify({
    '@context':'https://schema.org','@type':'NewsArticle',headline:title,datePublished:article.published_at||article.created_at,dateModified:article.published_at||article.created_at,
    url,publisher:{'@type':'Organization',name:'Vetor Global',url:SITE_URL},description,
...
const leituraVetor = article => {
  const item = noticiaApresentavel(article)
  const titulo = item.displayTitle || 'esta notícia'
  const t = normalizar(titulo)
  const resumo = item.displaySummary
  const categoria = normalizar(item.category?.slug || item.category?.name || '')

  let contexto = resumo
    ? resumo
    : `A notícia informa que ${titulo}. O fato está inserido no acompanhamento de ${item.category?.name || 'acontecimentos recentes'} pelo Vetor Global.`

  let importa = ''
  if (/(juros|fed|fomc|inflacao|ipca|banco central|selic|copom|dolar|cambio)/i.test(t)) {
    importa = 'A decisão ou dado pode alterar expectativas de juros, câmbio e preços dos ativos, por isso a reação dos mercados e as próximas sinalizações merecem atenção.'
  } else if (/(bitcoin|btc|ethereum|eth|solana|xrp|cripto|etf|blockchain)/i.test(t)) {
    importa = 'O fato pode afetar o sentimento, o fluxo de capital e a percepção de risco no mercado de ativos digitais.'
  } else if (/(guerra|ira|israel|ucrania|russia|china|eua|otan|geopolit|tarifa|sanções|sancoes)/i.test(t)) {
    importa = 'O desenvolvimento pode produzir efeitos além do fato inicial, especialmente sobre relações internacionais, comércio, energia e mercados.'
  } else if (/(ia|inteligencia artificial|tecnologia|chip|semicondutor|software)/i.test(t)) {
    importa = 'A notícia pode sinalizar mudanças em tecnologia, negócios, investimentos e adoção de novas soluções.'
  } else if (/(saude|saúde|hospital|medicina|vacina|doenca|doença|anvisa)/i.test(t)) {
    importa = 'O desdobramento pode afetar decisões de saúde pública, serviços, empresas ou a população diretamente envolvida.'
  } else if (/(esporte|futebol|tenis|tênis|basquete|atleta|torneio|jogo|campeonato)/i.test(t) || categoria === 'esportes') {
    importa = 'A decisão ou resultado influencia o calendário competitivo e os próximos compromissos do atleta ou da equipe.'
  } else {
    importa = `O principal impacto está relacionado aos próximos desdobramentos de ${item.category?.name || 'este tema'}, que podem mudar a leitura do cenário.`
  }

  let observar = []
  if (/(desiste|retira|abandona|pausa|afasta|cancel|adiamento|adiar)/i.test(t)) observar.push('próximos compromissos e eventual retomada')
  if (/(anuncia|decide|decisão|aprov|governo|congresso|lei|regulacao|regulação)/i.test(t)) observar.push('novas decisões, comunicados e medidas oficiais')
  if (/(mercado|bitcoin|btc|ethereum|eth|dolar|cambio|bolsa|acoes|ações|preco|preço)/i.test(t)) observar.push('reação dos preços, volumes e expectativas do mercado')
  if (/(guerra|ira|israel|ucrania|russia|china|tarifa|sancoes|sanções)/i.test(t)) observar.push('novos acontecimentos e possíveis efeitos internacionais')
  if (!observar.length) observar.push('novas informações e desdobramentos confirmados')
  if (observar.length < 3) observar.push('declarações ou dados que ajudem a medir o impacto do fato')
  observar = [...new Set(observar)].slice(0, 3)

  return { contexto, importa, observar }
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (!match) {
      ...
      document.title = 'Vetor Global — Informação que move decisões'
      return
    }
    const id = decodeURIComponent(match[1])
    let active = true
    setArticleDetailLoading(true)
    supabase.from('articles').select('id,title,summary,url,image_url,published_at,created_at,category:categories(id,name,slug),source:sources(name)').eq('id', id).maybeSingle().then(({data,error}) => {
      if (!active) return
      if (!error && data) { setArticleDetail(data); setArticleSeo(data) }
      else { setArticleDetail(null); setCanonical(SITE_URL + '/'); document.title='Notícia não encontrada | Vetor Global' }
  ...
  if (window.location.pathname.startsWith('/noticia/')) {
    if (articleDetailLoading) return <div className="vg-site"><main className="vg-main"><div className="vg-empty">Carregando notícia…</div></main></div>
    if (!articleDetail) return <div className="vg-site"><main className="vg-main"><div className="vg-empty">Notícia não encontrada.</div></main></div>
    const item = noticiaApresentavel(articleDetail)
    const leitura = leituraVetor(articleDetail)
    return <div className="vg-site"><header className="vg-header"><button className="vg-brand" onClick={() => window.location.href='/'} aria-label="Ir para o início"><span className="vg-logo">V</span><span><strong>Vetor Global</strong><small>Informações que movem decisões.</small></span></button><a className="vg-detail-top-back" href="/">← Voltar ao portal</a></header><main className="vg-main"><article className="vg-article-detail"><div className="vg-detail-breadcrumb">Notícia → contexto → impacto → cenário</div><span className="tag">{item.category?.name || 'Notícia'}</span><h1>{item.displayTitle}</h1><p className="vg-detail-subline">Notícia → contexto → impacto.</p><p className="vg-detail-meta">{item.displaySource} • {dataFormatada(item.published_at || item.created_at)}</p>{item.image_url && <img src={item.image_url} alt={item.displayTitle} />}{item.displaySummary && <p className="vg-detail-summary">{item.displaySummary}</p>}<section className="vg-reading"><div className="vg-reading-head"><span>LEITURA VETOR</span><strong>Entenda o fato, o impacto e o que observar.</strong></div><div className="vg-reading-grid"><div><h2>CONTEXTO</h2><p>{leitura.contexto}</p></div><div><h2>POR QUE IMPORTA</h2><p>{leitura.importa}</p></div><div><h2>O QUE OBSERVAR</h2><ul>{leitura.observar.map((item, i) => <li key={i}>{item}</li>)}</ul></div></div></section><p className="vg-detail-note">Análise editorial automatizada a partir dos dados disponíveis da notícia. Consulte a fonte original para o conteúdo completo.</p><section className="vg-source-box"><strong>Fonte original: {item.displaySource}</strong><p>A notícia completa pertence ao veículo de origem. O Vetor Global apresenta referência, contexto e leitura editorial e direciona você à fonte original.</p><a className="vg-detail-source" href={item.url} target="_blank" rel="noreferrer">Abrir fonte original em nova aba ↗</a><a className="vg-detail-back" href="/">← Continuar no Vetor Global</a></section></article></main><footer><strong>Vetor Global</strong><span>Notícias • Mercados • Cripto • Mundo</span></footer></div>
  }
