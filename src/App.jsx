...
export default function App() {
  ...
  if (window.location.pathname.startsWith('/noticia/')) {
    ...
    return <div className="vg-site"><header className="vg-header"><button className="vg-brand" onClick={() => window.location.href='/'} aria-label="Ir para o início"><span className="vg-logo">V</span><span><strong>Vetor Global</strong><small>Informações que movem decisões.</small></span></button></header><main className="vg-main"><article className="vg-article-detail"><span className="tag">{item.category?.name || 'Notícia'}</span><h1>{item.displayTitle}</h1><p className="vg-detail-meta">{item.displaySource} • {dataFormatada(item.published_at || item.created_at)}</p>{item.image_url && <img src={item.image_url} alt="" />}{item.displaySummary && <p className="vg-detail-summary">{item.displaySummary}</p>}<p className="vg-detail-note">Leia a matéria completa na fonte original.</p><a className="vg-detail-source" href={item.url} target="_blank" rel="noreferrer">Abrir fonte original →</a><br/><a className="vg-detail-back" href="/">← Voltar ao Vetor Global</a></article></main><footer><strong>Vetor Global</strong><span>Notícias • Mercados • Cripto • Mundo</span></footer></div>
  }
